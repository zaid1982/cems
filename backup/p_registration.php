    ...
    if (!isset($_SESSION['user_id'])) {
        ...
        if ($_POST['funct'] == 'update_consultant') {            
            ...
            if ($arrayParam['consAll_type'] == '1') {
                ...
                Class_db::getInstance()->db_insert('t_consultant_cems', array('consAll_id'=>$consAll_id, 'consultant_id'=>$consultant_id, 'wfTrans_id'=>$wfTrans_id, 'consCems_contactPerson'=>$_SESSION['user_id'], 'consCems_createdBy'=>$_SESSION['user_id'], 'dis_id'=>$dis_id));
            } else if ($arrayParam['consAll_type'] == '2') {
                $dis_id = Class_db::getInstance()->db_insert('t_dis', array());
                Class_db::getInstance()->db_insert('t_consultant_pems', array('consAll_id'=>$consAll_id, 'consultant_id'=>$consultant_id, 'wfTrans_id'=>$wfTrans_id, 'consPems_contactPerson'=>$_SESSION['user_id'], 'consPems_createdBy'=>$_SESSION['user_id'], 'dis_id'=>$dis_id));
            } else if ($arrayParam['consAll_type'] == '3') {
                $dis_id = Class_db::getInstance()->db_insert('t_dis', array());
                Class_db::getInstance()->db_insert('t_consultant_mobile', array('consAll_id'=>$consAll_id, 'consultant_id'=>$consultant_id, 'wfTrans_id'=>$wfTrans_id, 'consMobile_contactPerson'=>$_SESSION['user_id'], 'consMobile_createdBy'=>$_SESSION['user_id'], 'dis_id'=>$dis_id));
            }
            ...
            if (Class_db::getInstance()->db_count('vw_consultant_docs', array('consAll_id'=>$_POST['consAll_id'], 'document_uplname'=>$_FILES['mac_file_catalogue']['name'], 'documentName_id'=>'9')) > 0)
                throw new Exception('(ErrCode:5848) [' . __LINE__ . '] - Document Name already exist.', 32);
            $document_id = $fn_upload->upload_file('1', $_FILES['mac_file_catalogue'], 'Analyzer Manual / Catalogue', '9', '');
            Class_db::getInstance()->db_insert('t_consultant_doc', array('document_id'=>$document_id, 'documentName_id'=>'9', 'consAll_id'=>$_POST['consAll_id']));
            $result = $document_id;
        } else if ($_POST['funct'] == 'upload_analyzer_catalogue_mobile') {     
            if (empty($_POST['consAll_id']))    throw new Exception('(ErrCode:5815) [' . __LINE__ . '] - Parameter consAll_id empty.');
            if (Class_db::getInstance()->db_count('vw_consultant_docs', array('consAll_id'=>$_POST['consAll_id'], 'document_uplname'=>$_FILES['mam_file_catalogue']['name'], 'documentName_id'=>'9')) > 0)
                throw new Exception('(ErrCode:5848) [' . __LINE__ . '] - Document Name already exist.', 32);
            $document_id = $fn_upload->upload_file('1', $_FILES['mam_file_catalogue'], 'Analyzer Manual / Catalogue', '9', '');
            Class_db::getInstance()->db_insert('t_consultant_doc', array('document_id'=>$document_id, 'documentName_id'=>'9', 'consAll_id'=>$_POST['consAll_id']));
            $result = $document_id;
            ...
            $arr_consEmisCate = Class_db::getInstance()->db_select_colm ('t_consultant_emission_category', array('consAll_id'=>$_POST['mac_consAll_id']), 'consEmisCate_value');
            $arrPost_consEmisCate = (!empty($_POST['mac_consEmisCate_value'])) ? $_POST['mac_consEmisCate_value'] : array();
            if ($arr_consEmisCate != $arrPost_consEmisCate) {
                $arrDiff_consEmisCate1 = array_diff($arrPost_consEmisCate, $arr_consEmisCate);
                foreach($arrDiff_consEmisCate1 as $value) {
                    Class_db::getInstance()->db_insert('t_consultant_emission_category', array('consAll_id'=>$_POST['mac_consAll_id'], 'consEmisCate_value'=>$value));
                }
                $arrDiff_consEmisCate2 = array_diff($arr_consEmisCate, $arrPost_consEmisCate);
                if (count($arrDiff_consEmisCate2) > 0) {
                    Class_db::getInstance()->db_delete('t_consultant_emission_category', array('consAll_id'=>$_POST['mac_consAll_id'], 'consEmisCate_value'=>'('.  implode(',', $arrDiff_consEmisCate2).')'));
                }
            }     
            ...
            Class_db::getInstance()->db_update('t_consultant_cems', array('consCems_isInstall'=>$consCems_isInstall, 'consCems_isMaintain'=>$consCems_isMaintain, 'consCems_compStatus'=>$consCems_compStatus, 'consCems_type'=>$_POST['mac_consCems_type'],
                'consCems_modelNo'=>$_POST['mac_consCems_modelNo'], 'consCems_isNormalize'=>$_POST['mac_consCems_isNormalize'], 'consCems_correction'=>$_POST['mac_consCems_correction'], 'analyzerTechnique_id'=>$_POST['mac_analyzerTechnique_id'], 'consCems_brand'=>$_POST['mac_consCems_brand'],
            ...
            $arr_consQa = Class_db::getInstance()->db_select_colm ('t_consultant_qa', array('consAll_id'=>$_POST['mac_consAll_id'], 'qaQuarter_type'=>'1'), 'qaQuarter_id');
            $arrPost_consQa = (!empty($_POST['mac_q_qaQuarter_id'])) ? $_POST['mac_q_qaQuarter_id'] : array();
            if ($arr_consQa != $arrPost_consQa) {
                $arrDiff_consQa1 = array_diff($arrPost_consQa, $arr_consQa);
                foreach($arrDiff_consQa1 as $value) {
                    Class_db::getInstance()->db_insert('t_consultant_qa', array('consAll_id'=>$_POST['mac_consAll_id'], 'qaQuarter_type'=>'1', 'qaQuarter_id'=>$value));
                }
                $arrDiff_consQa2 = array_diff($arr_consQa, $arrPost_consQa);
                if (count($arrDiff_consQa2) > 0) {
                    Class_db::getInstance()->db_delete('t_consultant_qa', array('consAll_id'=>$_POST['mac_consAll_id'], 'qaQuarter_type'=>'1', 'qaQuarter_id'=>'('.  implode(',', $arrDiff_consQa2).')'));
                }
            }   
            Class_db::getInstance()->db_delete('t_consultant_qa', array('consAll_id'=>$_POST['mac_consAll_id'], 'qaQuarter_type'=>'2'));
            if (!empty($_POST['mac_y_qaQuarter_id']))
                Class_db::getInstance()->db_insert('t_consultant_qa', array('consAll_id'=>$_POST['mac_consAll_id'], 'qaQuarter_type'=>'2', 'qaQuarter_id'=>$_POST['mac_y_qaQuarter_id']));
            $consAll_qaFreqDaily = (!empty($_POST['mac_consAll_qaFreqDaily'])) ? $_POST['mac_consAll_qaFreqDaily'] : '';
            $consAll_qaMethod = (!empty($_POST['mac_consAll_qaMethod'])) ? $_POST['mac_consAll_qaMethod'] : '';
            $consAll_qaFreqQuarterly = (!empty($_POST['mac_consAll_qaFreqQuarterly'])) ? $_POST['mac_consAll_qaFreqQuarterly'] : '';
            $consAll_qaFreqYearly = (!empty($_POST['mac_consAll_qaFreqYearly'])) ? $_POST['mac_consAll_qaFreqYearly'] : '';
            Class_db::getInstance()->db_update('t_consultant_all', array('consAll_qaFreqYearly'=>$consAll_qaFreqYearly, 'consAll_qaFreqQuarterly'=>$consAll_qaFreqQuarterly, 'consAll_qaFreqDaily'=>$consAll_qaFreqDaily, 'consAll_qaMethod'=>$consAll_qaMethod, 'consAll_remark'=>$_POST['mac_wfTask_remark']), array('consAll_id'=>$_POST['mac_consAll_id']));
            ...
            $arr_consEmisCate = Class_db::getInstance()->db_select_colm ('t_consultant_emission_category', array('consAll_id'=>$_POST['map_consAll_id']), 'consEmisCate_value');
            $arrPost_consEmisCate = (!empty($_POST['map_consEmisCate_value'])) ? $_POST['map_consEmisCate_value'] : array();
            if ($arr_consEmisCate != $arrPost_consEmisCate) {
                $arrDiff_consEmisCate1 = array_diff($arrPost_consEmisCate, $arr_consEmisCate);
                foreach($arrDiff_consEmisCate1 as $value) {
                    Class_db::getInstance()->db_insert('t_consultant_emission_category', array('consAll_id'=>$_POST['map_consAll_id'], 'consEmisCate_value'=>$value));
                }
                $arrDiff_consEmisCate2 = array_diff($arr_consEmisCate, $arrPost_consEmisCate);
                if (count($arrDiff_consEmisCate2) > 0) {
                    Class_db::getInstance()->db_delete('t_consultant_emission_category', array('consAll_id'=>$_POST['map_consAll_id'], 'consEmisCate_value'=>'('.  implode(',', $arrDiff_consEmisCate2).')'));
                }
            }     
            ...
            $consPems_outsource = $_POST['map_dis_type'] == '2' ? $_POST['map_dis_outsource'] : '';
            ...
            $arr_consQa = Class_db::getInstance()->db_select_colm ('t_consultant_qa', array('consAll_id'=>$_POST['map_consAll_id'], 'qaQuarter_type'=>'1'), 'qaQuarter_id');
            $arrPost_consQa = (!empty($_POST['map_q_qaQuarter_id'])) ? $_POST['map_q_qaQuarter_id'] : array();
            if ($arr_consQa != $arrPost_consQa) {
                $arrDiff_consQa1 = array_diff($arrPost_consQa, $arr_consQa);
                foreach($arrDiff_consQa1 as $value) {
                    Class_db::getInstance()->db_insert('t_consultant_qa', array('consAll_id'=>$_POST['map_consAll_id'], 'qaQuarter_type'=>'1', 'qaQuarter_id'=>$value));
                }
                $arrDiff_consQa2 = array_diff($arr_consQa, $arrPost_consQa);
                if (count($arrDiff_consQa2) > 0) {
                    Class_db::getInstance()->db_delete('t_consultant_qa', array('consAll_id'=>$_POST['map_consAll_id'], 'qaQuarter_type'=>'1', 'qaQuarter_id'=>'('.  implode(',', $arrDiff_consQa2).')'));
                }
            }   
            Class_db::getInstance()->db_delete('t_consultant_qa', array('consAll_id'=>$_POST['map_consAll_id'], 'qaQuarter_type'=>'2'));
            if (!empty($_POST['map_y_qaQuarter_id']))
                Class_db::getInstance()->db_insert('t_consultant_qa', array('consAll_id'=>$_POST['map_consAll_id'], 'qaQuarter_type'=>'2', 'qaQuarter_id'=>$_POST['map_y_qaQuarter_id']));
            $consAll_qaFreqDaily = (!empty($_POST['map_consAll_qaFreqDaily'])) ? $_POST['map_consAll_qaFreqDaily'] : '';
            $consAll_qaMethod = (!empty($_POST['map_consAll_qaMethod'])) ? $_POST['map_consAll_qaMethod'] : '';
            $consAll_qaFreqQuarterly = (!empty($_POST['map_consAll_qaFreqQuarterly'])) ? $_POST['map_consAll_qaFreqQuarterly'] : '';
            $consAll_qaFreqYearly = (!empty($_POST['map_consAll_qaFreqYearly'])) ? $_POST['map_consAll_qaFreqYearly'] : '';
            Class_db::getInstance()->db_update('t_consultant_all', array('consAll_qaFreqYearly'=>$consAll_qaFreqYearly, 'consAll_qaFreqQuarterly'=>$consAll_qaFreqQuarterly, 'consAll_qaFreqDaily'=>$consAll_qaFreqDaily, 'consAll_qaMethod'=>$consAll_qaMethod, 'consAll_remark'=>$_POST['map_wfTask_remark']), array('consAll_id'=>$_POST['map_consAll_id']));
            ...
            $arr_consEmisCate = Class_db::getInstance()->db_select_colm ('t_consultant_emission_category', array('consAll_id'=>$_POST['mam_consAll_id']), 'consEmisCate_value');
            $arrPost_consEmisCate = (!empty($_POST['mam_consEmisCate_value'])) ? $_POST['mam_consEmisCate_value'] : array();
            if ($arr_consEmisCate != $arrPost_consEmisCate) {
                $arrDiff_consEmisCate1 = array_diff($arrPost_consEmisCate, $arr_consEmisCate);
                foreach($arrDiff_consEmisCate1 as $value) {
                    Class_db::getInstance()->db_insert('t_consultant_emission_category', array('consAll_id'=>$_POST['mam_consAll_id'], 'consEmisCate_value'=>$value));
                }
                $arrDiff_consEmisCate2 = array_diff($arr_consEmisCate, $arrPost_consEmisCate);
                if (count($arrDiff_consEmisCate2) > 0) {
                    Class_db::getInstance()->db_delete('t_consultant_emission_category', array('consAll_id'=>$_POST['mam_consAll_id'], 'consEmisCate_value'=>'('.  implode(',', $arrDiff_consEmisCate2).')'));
                }
            }     
            ...
            Class_db::getInstance()->db_update('t_consultant_mobile', array('consMobile_refMethod'=>$consMobile_refMethod, 'consMobile_compStatus'=>$consMobile_compStatus, 'consMobile_type'=>$_POST['mam_consMobile_type'], 'consMobile_correction'=>$_POST['mam_consMobile_correction'],
                'consMobile_modelNo'=>$_POST['mam_consMobile_modelNo'], 'consMobile_isNormalize'=>$_POST['mam_consMobile_isNormalize'], 'analyzerTechnique_id'=>$_POST['mam_analyzerTechnique_id'], 'consMobile_brand'=>$_POST['mam_consMobile_brand'],
            ...
            if (empty($_POST['mac_consParam_rangeFromHigh']))   throw new Exception('(ErrCode:5820) [' . __LINE__ . '] - Field Specified Range High From empty.', 32); 
            if (empty($_POST['mac_consParam_rangeToHigh']))     throw new Exception('(ErrCode:5846) [' . __LINE__ . '] - Field Specified Range High To empty.', 32); 
            if (empty($_POST['mac_consParam_rangeFromLow']))    throw new Exception('(ErrCode:5856) [' . __LINE__ . '] - Field Specified Range Low From empty.', 32); 
            if (empty($_POST['mac_consParam_rangeToLow']))      throw new Exception('(ErrCode:5857) [' . __LINE__ . '] - Field Specified Range Low To empty.', 32); 
            if (empty($_POST['mac_consParam_reference']))       throw new Exception('(ErrCode:5821) [' . __LINE__ . '] - Field Reference Traceability empty.', 32); 
            if (empty($_POST['mac_consParam_dataGeneration']))  throw new Exception('(ErrCode:5847) [' . __LINE__ . '] - Field Data Generation empty.', 32); 
            if (empty($_POST['mac_consParam_method']))          throw new Exception('(ErrCode:5853) [' . __LINE__ . '] - Field Method empty.', 32); 
            ...
            $result = Class_db::getInstance()->db_insert('t_consultant_parameter', array('consAll_id'=>$_POST['mac_consAll_id'], 'inputParam_id'=>$_POST['mac_inputParam_id'], 'consParam_rangeFromHigh'=>$_POST['mac_consParam_rangeFromHigh'], 'consParam_rangeToHigh'=>$_POST['mac_consParam_rangeToHigh'],
                'consParam_rangeFromLow'=>$_POST['mac_consParam_rangeFromLow'], 'consParam_rangeToLow'=>$_POST['mac_consParam_rangeToLow'], 'consParam_reference'=>$_POST['mac_consParam_reference'], 'consParam_dataGeneration'=>$_POST['mac_consParam_dataGeneration'], 'consParam_method'=>$_POST['mac_consParam_method']));
        } else if ($_POST['funct'] == 'save_consultant_parameter_pems') {
            if (empty($_POST['map_consAll_id']))                throw new Exception('(ErrCode:5817) [' . __LINE__ . '] - Parameter consAll_id empty.'); 
            if (empty($_POST['map_inputParam_id']))             throw new Exception('(ErrCode:5819) [' . __LINE__ . '] - Field Input Parameter empty.', 32); 
            if (empty($_POST['map_consParam_rangeFromHigh']))   throw new Exception('(ErrCode:5820) [' . __LINE__ . '] - Field Specified Range High From empty.', 32); 
            if (empty($_POST['map_consParam_rangeToHigh']))     throw new Exception('(ErrCode:5846) [' . __LINE__ . '] - Field Specified Range High To empty.', 32); 
            if (empty($_POST['map_consParam_rangeFromLow']))    throw new Exception('(ErrCode:5856) [' . __LINE__ . '] - Field Specified Range Low From empty.', 32); 
            if (empty($_POST['map_consParam_rangeToLow']))      throw new Exception('(ErrCode:5857) [' . __LINE__ . '] - Field Specified Range Low To empty.', 32); 
            if (empty($_POST['map_consParam_reference']))       throw new Exception('(ErrCode:5821) [' . __LINE__ . '] - Field Reference Traceability empty.', 32); 
            if (empty($_POST['map_consParam_dataGeneration']))  throw new Exception('(ErrCode:5847) [' . __LINE__ . '] - Field Data Generation empty.', 32);  
            if (empty($_POST['map_consParam_method']))          throw new Exception('(ErrCode:5853) [' . __LINE__ . '] - Field Method empty.', 32); 
            if (Class_db::getInstance()->db_count('t_consultant_parameter', array('inputParam_id'=>$_POST['map_inputParam_id'], 'consAll_id'=>$_POST['map_consAll_id'])) > 0)
                throw new Exception('(ErrCode:5845) [' . __LINE__ . '] - Input Parameter already exist.', 32);
            $result = Class_db::getInstance()->db_insert('t_consultant_parameter', array('consAll_id'=>$_POST['map_consAll_id'], 'inputParam_id'=>$_POST['map_inputParam_id'], 'consParam_rangeFromHigh'=>$_POST['map_consParam_rangeFromHigh'], 'consParam_rangeToHigh'=>$_POST['map_consParam_rangeToHigh'],
                'consParam_rangeFromLow'=>$_POST['map_consParam_rangeFromLow'], 'consParam_rangeToLow'=>$_POST['map_consParam_rangeToLow'], 'consParam_reference'=>$_POST['map_consParam_reference'], 'consParam_dataGeneration'=>$_POST['map_consParam_dataGeneration'], 'consParam_method'=>$_POST['map_consParam_method']));
            ...
            if (empty($_POST['mam_consParam_rangeFromHigh']))   throw new Exception('(ErrCode:5820) [' . __LINE__ . '] - Field Specified Range High From empty.', 32); 
            if (empty($_POST['mam_consParam_rangeToHigh']))     throw new Exception('(ErrCode:5846) [' . __LINE__ . '] - Field Specified Range High To empty.', 32); 
            if (empty($_POST['mam_consParam_rangeFromLow']))    throw new Exception('(ErrCode:5856) [' . __LINE__ . '] - Field Specified Range Low From empty.', 32); 
            if (empty($_POST['mam_consParam_rangeToLow']))      throw new Exception('(ErrCode:5857) [' . __LINE__ . '] - Field Specified Range Low To empty.', 32); 
            if (empty($_POST['mam_consParam_reference']))       throw new Exception('(ErrCode:5821) [' . __LINE__ . '] - Field Reference Traceability empty.', 32); 
            if (empty($_POST['mam_consParam_dataGeneration']))  throw new Exception('(ErrCode:5847) [' . __LINE__ . '] - Field Data Generation empty.', 32);  
            if (empty($_POST['mam_consParam_method']))          throw new Exception('(ErrCode:5853) [' . __LINE__ . '] - Field Method empty.', 32); 
            if (Class_db::getInstance()->db_count('t_consultant_parameter', array('inputParam_id'=>$_POST['mam_inputParam_id'], 'consAll_id'=>$_POST['mam_consAll_id'])) > 0)
                throw new Exception('(ErrCode:5845) [' . __LINE__ . '] - Input Parameter already exist.', 32);
            $result = Class_db::getInstance()->db_insert('t_consultant_parameter', array('consAll_id'=>$_POST['mam_consAll_id'], 'inputParam_id'=>$_POST['mam_inputParam_id'], 'consParam_rangeFromHigh'=>$_POST['mam_consParam_rangeFromHigh'], 'consParam_rangeToHigh'=>$_POST['mam_consParam_rangeToHigh'],
                'consParam_rangeFromLow'=>$_POST['mam_consParam_rangeFromLow'], 'consParam_rangeToLow'=>$_POST['mam_consParam_rangeToLow'], 'consParam_reference'=>$_POST['mam_consParam_reference'], 'consParam_dataGeneration'=>$_POST['mam_consParam_dataGeneration'], 'consParam_method'=>$_POST['mam_consParam_method']));
            ...
            if (empty($_POST['mac_certificate_dateExpired']))   throw new Exception('(ErrCode:5824) [' . __LINE__ . '] - Field Expired Date empty.', 32); 
            $document_id = !empty($_FILES['mac_file_certificate']['name']) ? $fn_upload->upload_file('1', $_FILES['mac_file_certificate'], 'Analyzer Certificate', '10', '') : '';
            $certificate_id = Class_db::getInstance()->db_insert('t_certificate', array('consAll_id'=>$_POST['mac_consAll_id'], 'certificate_no'=>$_POST['mac_certificate_no'], 'certIssuer_id'=>$_POST['mac_certIssuer_id'],
                'certificate_dateExpired'=>$_POST['mac_certificate_dateExpired'], 'certificate_timeSubmit'=>'Now()', 'certificate_timeFinish'=>'Now()', 'document_id'=>$document_id));
            ...
            if (empty($_POST['mam_certificate_dateExpired']))   throw new Exception('(ErrCode:5824) [' . __LINE__ . '] - Field Expired Date empty.', 32); 
            $document_id = !empty($_FILES['mam_file_certificate']['name']) ? $fn_upload->upload_file('1', $_FILES['mam_file_certificate'], 'Analyzer Certificate', '10', '') : '';
            $certificate_id = Class_db::getInstance()->db_insert('t_certificate', array('consAll_id'=>$_POST['mam_consAll_id'], 'certificate_no'=>$_POST['mam_certificate_no'], 'certIssuer_id'=>$_POST['mam_certIssuer_id'],
                'certificate_dateExpired'=>$_POST['mam_certificate_dateExpired'], 'certificate_timeSubmit'=>'Now()', 'certificate_timeFinish'=>'Now()', 'document_id'=>$document_id));
            ...
            $document_id = !empty($_FILES['mac_consPers_document']['name']) ? $fn_upload->upload_file('1', $_FILES['mac_consPers_document'], 'Personnel Supportive Document', '12', '') : '';
            ...
            $personnel_id = '';
            $personnel = Class_db::getInstance()->db_select_single('t_personnel', array('personnel_icNo'=>$_POST['map_personnel_icNo']));if (Class_db::getInstance()->db_count('t_consultant_personnel', array('personnel_id'=>$personnel['personnel_id'], 'consAll_id'=>$_POST['map_consAll_id'])) > 0)
                throw new Exception('(ErrCode:5850) [' . __LINE__ . '] - IC / Passport No. already exist in your list.', 32); 
            if (!empty($personnel)) {
            ...
            $document_id = !empty($_FILES['map_consPers_document']['name']) ? $fn_upload->upload_file('1', $_FILES['map_consPers_document'], 'Personnel Supportive Document', '12', '') : '';
            ...
            $personnel_id = '';
            $personnel = Class_db::getInstance()->db_select_single('t_personnel', array('personnel_icNo'=>$_POST['mam_personnel_icNo']));if (Class_db::getInstance()->db_count('t_consultant_personnel', array('personnel_id'=>$personnel['personnel_id'], 'consAll_id'=>$_POST['mam_consAll_id'])) > 0)
                throw new Exception('(ErrCode:5850) [' . __LINE__ . '] - IC / Passport No. already exist in your list.', 32); 
            if (!empty($personnel)) {
            ...
            $document_id = !empty($_FILES['mam_consPers_document']['name']) ? $fn_upload->upload_file('1', $_FILES['mam_consPers_document'], 'Personnel Supportive Document', '12', '') : '';
            ...
            if (empty($_POST['iin_address_line1']))         throw new Exception('(ErrCode:5803) [' . __LINE__ . '] - Field Registered Address empty.', 32); 
            if (empty($_POST['iin_address_postcode']))      throw new Exception('(ErrCode:5804) [' . __LINE__ . '] - Field Registered Postcode empty.', 32); 
            if (empty($_POST['iin_city_id']))               throw new Exception('(ErrCode:5805) [' . __LINE__ . '] - Field Registered City empty.', 32); 
            if (empty($_POST['iin_wfGroup_phoneNo']))       throw new Exception('(ErrCode:5809) [' . __LINE__ . '] - Field Phone No. empty.', 32); 
            if (empty($_POST['iin_industrial_id']))         throw new Exception('(ErrCode:5843) [' . __LINE__ . '] - Parameter industrial_id empty.', 32); 
            if (empty($_POST['iin_industrial_totalStack'])) throw new Exception('(ErrCode:5844) [' . __LINE__ . '] - Parameter industrial_totalStack empty.', 32); 
            $wf_group = Class_db::getInstance()->db_select_single('wf_group', array('wfGroup_id'=>$_POST['iin_wfGroup_id']), NULL, 1);
            $wf_group_profile = Class_db::getInstance()->db_select_single('wf_group_profile', array('wfGroupProfile_id'=>$wf_group['wfGroupProfile_id']), NULL, 1);
            $wfGroup_address = '';
            if ($wf_group_profile['wfGroup_address'] == '') {
                $wfGroup_address = Class_db::getInstance()->db_insert('address', array('address_line1'=>$_POST['iin_address_line1'], 'address_postcode'=>$_POST['iin_address_postcode'], 'city_id'=>$_POST['iin_city_id']));
            } else {
                $wfGroup_address = $wf_group_profile['wfGroup_address'];
                Class_db::getInstance()->db_update('address', array('address_line1'=>$_POST['iin_address_line1'], 'address_postcode'=>$_POST['iin_address_postcode'], 'city_id'=>$_POST['iin_city_id']), array('address_id'=>$wf_group_profile['wfGroup_address']));
            }
            ...
            $mail_city_id = '';
            if ($same_address == '1') {
                $mail_address_line1 = $_POST['iin_address_line1'];
                $mail_address_postcode = $_POST['iin_address_postcode'];
                $mail_city_id = $_POST['iin_city_id'];
            } else {                
            ...
            if ($wf_group['wfGroup_isFirstTime'] == '1') {
                Class_db::getInstance()->db_update('wf_group_profile', array('wfGroup_address'=>$wfGroup_address, 'wfGroup_address_mail'=>$wfGroup_address_mail, 'wfGroup_phoneNo'=>$_POST['iin_wfGroup_phoneNo'],
                    'wfGroup_faxNo'=>$_POST['iin_wfGroup_faxNo'], 'wfGroup_address_same'=>$same_address), array('wfGroupProfile_id'=>$wf_group['wfGroupProfile_id']));
                Class_db::getInstance()->db_update('wf_group', array('wfGroup_isFirstTime'=>'0'), array('wfGroup_id'=>$wf_group['wfGroup_id']));
            } else {
                Class_db::getInstance()->db_update('wf_group_profile', array('wfGroupProfile_status'=>'3', 'wfGroupProfile_timeEnd'=>'Now()'), array('wfGroupProfile_id'=>$wf_group['wfGroupProfile_id']));
                $wfGroupProfile_id = Class_db::getInstance()->db_insert('wf_group_profile', array('wfGroup_address'=>$wfGroup_address, 'wfGroup_address_mail'=>$wfGroup_address_mail, 'wfGroup_phoneNo'=>$_POST['iin_wfGroup_phoneNo'],
            ...
            Class_db::getInstance()->db_update('t_industrial_all', array('sourceActivity_id'=>$_POST['mce_sourceActivity_id'], 'sourceCapacity_id'=>(isset($_POST['mce_sourceCapacity_id'])?$_POST['mce_sourceCapacity_id']:''), 'fuelType_id'=>$_POST['mce_fuelType_id'],
                ...
                'indAll_pressure'=>$_POST['mce_indAll_pressure'], 'consultant_id'=>$_POST['mce_consultant_id'], 'consAll_id'=>(isset($_POST['mce_consAll_id'])?$_POST['mce_consAll_id']:''), 'indAll_remark'=>$_POST['mce_wfTask_remark'],
                ), array('indAll_id'=>$_POST['mce_indAll_id']));
            ...
            Class_db::getInstance()->db_update('t_industrial_all', array('sourceActivity_id'=>$_POST['mpe_sourceActivity_id'], 'sourceCapacity_id'=>(isset($_POST['mpe_sourceCapacity_id'])?$_POST['mpe_sourceCapacity_id']:''), 'fuelType_id'=>$_POST['mpe_fuelType_id'],
                ...
                ), array('indAll_id'=>$_POST['mpe_indAll_id']));
            ...
            $document_id = !empty($_FILES['mce_file_written']['name']) ? $fn_upload->upload_file('1', $_FILES['mce_file_written'], 'CEMS Writen Approval', $_POST['mce_written_type'], '') : '';
            $result = Class_db::getInstance()->db_insert('t_industrial_written', array('indAll_id'=>$_POST['mce_indAll_id'], 'documentName_id'=>$_POST['mce_written_type'], 'indWritten_referenceNo'=>$_POST['mce_indWritten_referenceNo'],
                'indWritten_dateReference'=>$_POST['mce_indWritten_dateReference'], 'document_id'=>$document_id));
            ...
            $document_id = !empty($_FILES['mpe_file_written']['name']) ? $fn_upload->upload_file('1', $_FILES['mpe_file_written'], 'PEMS Writen Approval', $_POST['mpe_written_type'], '') : '';
            $result = Class_db::getInstance()->db_insert('t_industrial_written', array('indAll_id'=>$_POST['mpe_indAll_id'], 'documentName_id'=>$_POST['mpe_written_type'], 'indWritten_referenceNo'=>$_POST['mpe_indWritten_referenceNo'],
                'indWritten_dateReference'=>$_POST['mpe_indWritten_dateReference'], 'document_id'=>$document_id));
            ...
            $document_id = !empty($_FILES['mce_file_document']['name']) ? $fn_upload->upload_file('1', $_FILES['mce_file_document'], 'CEMS Document', $_POST['mce_document_type'], '') : '';
            ...
            $document_id = !empty($_FILES['mpe_file_document']['name']) ? $fn_upload->upload_file('1', $_FILES['mpe_file_document'], 'PEMS Document', $_POST['mpe_document_type'], '') : '';
            ...
            $document_id = !empty($_FILES['mce_file_docNormalize']['name']) ? $fn_upload->upload_file('1', $_FILES['mce_file_docNormalize'], 'CEMS Normalization Document', $_POST['mce_docNormalize_type'], '') : '';
            ...
            $document_id = !empty($_FILES['mpe_file_docNormalize']['name']) ? $fn_upload->upload_file('1', $_FILES['mpe_file_docNormalize'], 'PEMS Normalization Document', $_POST['mpe_docNormalize_type'], '') : '';
            ...
            if (empty($_POST['mbc_wfTask_id']))      throw new Exception('(ErrCode:5816) [' . __LINE__ . '] - Parameter wfTask_id empty.');  
            Class_db::getInstance()->db_update('wf_task', array('wfTask_statusSave'=>empty($_POST['mbc_result'])?'':$_POST['mbc_result'], 'wfTask_remark'=>$_POST['mbc_wfTask_verify']), array('wfTask_id'=>$_POST['mbc_wfTask_id']));
            $result = '1';
        } else {
