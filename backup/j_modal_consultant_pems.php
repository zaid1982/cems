<script type="text/javascript">  
    ...
    var map_otable_consParam;
    var data_map_consParam;
    ...
    $(document).ready(function () {
        ...
        $('#form_map_2_1').bootstrapValidator({
            ...
            fields: {  
                ...
                map_consPems_mobileConsultant : {
                    validators: {
                        notEmpty: {
                            message: 'Mobile-CEMS/Portable Consultant is required'
                        }
                    }
                },
                map_consPems_mobileCems : {
                    validators: {
                        notEmpty: {
                            message: 'Mobile-CEMS/Portable Model No. is required'
                        }
                    }
                },
        ...
        $('#form_map_2_3').bootstrapValidator({
            feedbackIcons: {
                valid: 'glyphicon glyphicon-ok',
                invalid: 'glyphicon glyphicon-remove',
                validating: 'glyphicon glyphicon-refresh'
            ...
            fields: {  
                ...
                map_consParam_rangeToHigh : {
                    validators: {
                        ...
                        callback: {
                            ...
                            callback: function (value, validator, $field) {
                                var value_from = parseFloat($('#map_consParam_rangeFromHigh').val());
                                return {
                ...
                map_consParam_rangeToLow : {
                    validators: {
                        ...
                        callback: {
                            message: 'Specified Range To must be greater than Specified Range From and less than 1,000',
                            callback: function (value, validator, $field) {
                                var value_from = parseFloat($('#map_consParam_rangeFromLow').val());
                                return {
                                    valid: parseFloat(value) > (isNaN(value_from)?0:value_from) && parseFloat(value) < 1000,
                                    message: 'Specified Range To must be greater than ' + (isNaN(value_from)?0:value_from) + ' and less than 1,000'
                                };
                ...
                map_consParam_reference : {
                    validators: {
                        notEmpty: {
                            message: 'Reference Traceability is required'
                        },
                        stringLength : {
                            max : 100,
        ...
        }).on('change', 'map_consParam_rangeFromHigh', function(e) {
            $('#form_map_2_3').bootstrapValidator('revalidateField', 'map_consParam_rangeToHigh');
        }).on('change', 'map_consParam_rangeFromLow', function(e) {
            $('#form_map_2_3').bootstrapValidator('revalidateField', 'map_consParam_rangeToLow');
        });  
        
        $('#map_consParam_rangeFromHigh').on('keyup', function () {
            $('#form_map_2_3').bootstrapValidator('revalidateField', 'map_consParam_rangeToHigh');
        });
        
        $('#map_consParam_rangeFromLow').on('keyup', function () {
            $('#form_map_2_3').bootstrapValidator('revalidateField', 'map_consParam_rangeToLow');
        }); 
        ...
        $('#form_map_4').find('[name="map_consAll_qaFreqQuarterly"]').on('click', function () { 
            if ($(this).val() == '2') {
                $('#form_map_4').bootstrapValidator('enableFieldValidators', 'map_q_qaQuarter_id[]', false);
                $('.map_q_qaQuarter').prop('checked', false).prop('disabled', true);
            } else {
                $('#form_map_4').bootstrapValidator('enableFieldValidators', 'map_q_qaQuarter_id[]', true);
                $('.map_q_qaQuarter').prop('disabled', false);
                f_map_set_quarterRATA();
            }
            $('#form_map_4').bootstrapValidator('revalidateField', 'map_q_qaQuarter_id[]');
        });
        
        $('#form_map_4').find('[name="map_consAll_qaFreqYearly"]').on('click', function () { 
            if ($(this).val() == '2') {
                $('#form_map_4').bootstrapValidator('enableFieldValidators', 'map_y_qaQuarter_id', false);
                $("input[name='map_y_qaQuarter_id']").prop('checked', false).prop('disabled', true);            
            } else {
                $('#form_map_4').bootstrapValidator('enableFieldValidators', 'map_y_qaQuarter_id', true);
                $("input[name='map_y_qaQuarter_id']").prop('disabled', false);  
                f_map_set_yearRATA();
            }
            $('#form_map_4').bootstrapValidator('revalidateField', 'map_y_qaQuarter_id');
        });
        
        $('.map_q_qaQuarter').on('click', function () {
            f_map_set_yearRATA();
        });
        
        $('#form_map_4').find('[name="map_y_qaQuarter_id"]').on('click', function () { 
            f_map_set_quarterRATA();
        });
...
//                            if (f_submit_forms('form_map,#form_map_2_1,#form_map_2_5,#form_map_4,#form_map_6', 'p_registration', 'Data successfully saved.')) {
//                                if (map_otable == 'cps')
//                                    f_table_cps ();
//                            }
...
//                }, 300000);
        ...
        $('#map_btn_submit').on('click', function () {            
            ...
            if (data_map_consParam.length == 0) {
                $('#map_wizard').wizard('selectedItem', { step:2 });
                $('#map_btn_add_parameter').focus();
                f_notify(2, 'Error', 'Please provide Parameters and Specified Range!');    
                return false;
            } else {
                var arrStr_sourceActivity = '';
                $.each($("input[name='map_sourceActivity_id[]']:checked"), function(){     
                ...
                $.each(arr_inputParam, function(u){     
                    is_exist = false;
                    $.each(data_map_consParam, function(v){     
                        if (data_map_consParam[v].inputParam_id == arr_inputParam[u].inputParam_id)
                            is_exist = true;
                    });
                    if (!is_exist) {
            ...
            bootstrapValidator = $("#form_map_4").data('bootstrapValidator');
            bootstrapValidator.validate();
            if (!bootstrapValidator.isValid()) {         
            ...
            bootstrapValidator.validate();
            if (!bootstrapValidator.isValid()) {         
                $('#map_wizard').wizard('selectedItem', { step:6 });
                f_notify(2, 'Error', errMsg_validation);    
                return false;
            }
            $("#map_funct").val('save_consultant_pems');
            $('#map_wfTask_remark').val($('[name="map_snote_wfTask_remark"]').summernote('code'));
            if (f_submit_forms('form_map,#form_map_2_1,#form_map_2_5,#form_map_4,#form_map_6', 'p_registration')) {
                $.SmartMessageBox({
        ...
        $('#map_btn_add_parameter').on('click', function () {
            var bootstrapValidator = $("#form_map_2_3").data('bootstrapValidator');
            bootstrapValidator.validate();
            if (bootstrapValidator.isValid()) {         
                $('#map_funct').val('save_consultant_parameter_pems');
                $('#modal_waiting').on('shown.bs.modal', function(e){    
                    if (f_submit_forms('form_map,#form_map_2_3', 'p_registration', 'Input Parameter successfully added.')) {
                        $('#form_map_2_3').bootstrapValidator('resetForm', true);
                        data_map_consParam = f_get_general_info_multiple('dt_consultant_parameter', {consAll_id:$('#map_consAll_id').val()}, '', '', 'consParam_id');
                        f_dataTable_draw(map_otable_consParam, data_map_consParam, 'datatable_map_consParam', 5);
                    }
                    $('#modal_waiting').modal('hide');
                    $(this).unbind(e);
                }).modal('show'); 
        ...
        var datatable_map_consParam = undefined; 
        map_otable_consParam = $('#datatable_map_consParam').DataTable({
            "paging": false,
            "ordering": false,
            "autoWidth": false,
            "info": false,
            ...
            "preDrawCallback": function () {
                if (!datatable_map_consParam) {
                    datatable_map_consParam = new ResponsiveDatatablesHelper($('#datatable_map_consParam'), breakpointDefinition);
                }
            },
            "rowCallback": function (nRow, aData, index) {
                datatable_map_consParam.createExpandIcon(nRow);
                var info = map_otable_consParam.page.info();
                $('td', nRow).eq(0).html(info.page * info.length + (index + 1));
            },
            "drawCallback": function (oSettings) {
                datatable_map_consParam.respond();
            },
            "aoColumns":
                [
                    {mData: null},
                    ...
                    {mData: null, sClass: 'text-center',
                        mRender: function (data, type, row) {
                            $label = '<button type="button" class="btn btn-danger btn-xs map_hideView" title="Delete" onclick="f_map_delete_consParam ('+row.consParam_id+');"><i class="fa fa-trash-o"></i></button>';
                            return $label;
                        }
                    }
                ]
    ...
    function f_map_delete_consParam (consParam_id) {
        $('#modal_waiting').on('shown.bs.modal', function(e){   
    ...
    function f_map_refresh_inputParameter() {        
        var arrStr_sourceActivity = '';
        $.each($("input[name='map_sourceActivity_id[]']:checked"), function(){     
            arrStr_sourceActivity += ',' + $(this).val();
        });
        var arr_inputParam = f_get_general_info_multiple('vw_pub_group_inputParam', {}, {arr_sourceActivity_id:arrStr_sourceActivity});
        get_option_data('map_inputParam_id', arr_inputParam, 'inputParam_id', 'inputParam_desc', ' ');
    }
    
    function f_map_set_yearRATA() {
        var searchIDs = $("input[name='map_q_qaQuarter_id[]']:checked").map(function(){
            return $(this).val();
        }).get();
        if (searchIDs.length == 3) {
            if ($('input[name="map_consAll_qaFreqQuarterly"]:checked').val() == '1' && $('input[name="map_consAll_qaFreqYearly"]:checked').val() == '1') {
                for(var n=1; n<=4; n++) {
                    if (!$("input[name='map_q_qaQuarter_id[]'][value="+n+"]").is(':checked'))
                        $("input[name='map_y_qaQuarter_id'][value="+n+"]").prop('checked', true);
                }
                $('#form_map_4').bootstrapValidator('revalidateField', 'map_y_qaQuarter_id');
            }
        }
    ...
    function f_map_set_quarterRATA() {
        var searchIDs = $("input[name='map_q_qaQuarter_id[]']:checked").map(function(){
            return $(this).val();
        }).get();
        if (searchIDs.length <= 3) {
            if ($('input[name="map_consAll_qaFreqQuarterly"]:checked').val() == '1' && $('input[name="map_consAll_qaFreqYearly"]:checked').val() == '1') {
                for(var n=1; n<=4; n++) {
                    $("input[name='map_q_qaQuarter_id[]'][value="+n+"]").prop('checked', false);
                    if (n != $('input[name="map_y_qaQuarter_id"]:checked').val())
                        $("input[name='map_q_qaQuarter_id[]'][value="+n+"]").prop('checked', true);
                }
                $('#form_map_4').bootstrapValidator('revalidateField', 'map_q_qaQuarter_id[]');
            }
        }
        ...
        $('#modal_waiting').on('shown.bs.modal', function(e){            
            if (map_1st_load) {                           
                ...
                $.each(source_activity, function(u){
                    ...
                    html += '<input type="checkbox" class="checkbox" name="map_sourceActivity_id[]" value="'+source_activity[u].sourceActivity_id+'" onclick="f_map_refresh_inputParameter();">';
            ...
            $('#form_map,#form_map_1,#form_map_2_1,#form_map_2_3,#form_map_2_5,#form_map_3,#form_map_4,#form_map_5,#form_map_6').trigger('reset');
            $('#form_map_2_1').bootstrapValidator('resetForm', true);
            $('#form_map_2_3').bootstrapValidator('resetForm', true);
            $('#form_map_2_5').bootstrapValidator('resetForm', true);
            $('#form_map_3').bootstrapValidator('resetForm', true);
            $('#form_map_4').bootstrapValidator('resetForm', true);
            $('#form_map_5').bootstrapValidator('resetForm', true);
            ...
            $('#form_map,#form_map_1,#form_map_2_1,#form_map_2_3,#form_map_2_5,#form_map_3,#form_map_4,#form_map_5,#form_map_6').find('input, textarea, select').prop('disabled',false);
            $('.map_hideView').show();
            $('.map_disView,#map_dis_outsource,#map_consPems_outsource').attr('disabled',true);
            $('#map_alert_box, .map_checkView').hide();
            ...
            get_option('map_inputParam_id', status, 't_input_parameter', 'inputParam_id', 'inputParam_desc', 'inputParam_status', ' ', 'ref_id');
            ...
            // ---------------- \\
            var consultant_emission_category = f_get_general_info_multiple('t_consultant_emission_category', {consAll_id:$('#map_consAll_id').val()});
            $.each(consultant_emission_category, function(u){
                $("input[name='map_consEmisCate_value[]'][value=" + consultant_emission_category[u].consEmisCate_value + "]").prop('checked', true);
            });
            // ---------------- \\
            ...
            f_map_refresh_inputParameter();
            // ---------------- \\
            $('#map_dis_outsource').attr('disabled', $('#map_dis_type').val() == '2' ? false : true);
            // ---------------- \\
            $('.map_q_qaQuarter').prop('disabled', consultant_pems.consAll_qaFreqQuarterly == '1' ? false : true);
            $("input[name='map_y_qaQuarter_id']").prop('disabled', consultant_pems.consAll_qaFreqYearly == '1' ? false : true);
            $("input[name='map_consAll_qaFreqDaily'][value=" + consultant_pems.consAll_qaFreqDaily + "]").prop('checked', true);
            $("input[name='map_consAll_qaMethod'][value=" + consultant_pems.consAll_qaMethod + "]").prop('checked', true);
            $("input[name='map_consAll_qaFreqQuarterly'][value=" + consultant_pems.consAll_qaFreqQuarterly + "]").prop('checked', true);
            $("input[name='map_consAll_qaFreqYearly'][value=" + consultant_pems.consAll_qaFreqYearly + "]").prop('checked', true);
            var consultant_qa = f_get_general_info_multiple('t_consultant_qa', {consAll_id:$('#map_consAll_id').val()});
            $.each(consultant_qa, function(u){
                if (consultant_qa[u].qaQuarter_type == '1')
                    $("input[name='map_q_qaQuarter_id[]'][value=" + consultant_qa[u].qaQuarter_id + "]").prop('checked', true);
                else if (consultant_qa[u].qaQuarter_type == '2')
                    $("input[name='map_y_qaQuarter_id'][value=" + consultant_qa[u].qaQuarter_id + "]").prop('checked', true);
            });
            // ---------------- \\
            ...
            if (map_load_type >= 3) {
                map_total_section = [];
                $('#form_map,#form_map_1,#form_map_2_1,#form_map_2_3,#form_map_2_5,#form_map_3,#form_map_4,#form_map_5,#form_map_6').find('input, textarea, select').prop('disabled',true);
