...
class Class_db{
    ...
    public function db_old_data($state_code, $premise_id, $data_date, $input_param) {   
        try {
            $config = parse_ini_file('../library/config.ini');
            $dbhost = $config['dbhost'];    
            $dbname = 'data_'.$state_code.'_'. substr($data_date, 0, 4);  
            $this->log_debug(__FUNCTION__, __LINE__, 'Connecting database '.$dbname.' for old data .....');
            $db_old = new PDO("mysql:host=$dbhost;dbname=$dbname", $config['username'], $config['password']);
            $db_old->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
            $tablename = $input_param == 'opacity' ? 'tbl_'.$premise_id.'_01' : 'tbl_'.$premise_id.'_30';
            $this->log_debug(__FUNCTION__, __LINE__, 'Get data from table '.$tablename.' .....');
            $sql = "SELECT stack_id, 
                DATE_ADD(
                    DATE_ADD(
                        DATE('1899-12-31'), INTERVAL FLOOR(rdt) DAY
                    ), INTERVAL ROUND(86400*(rdt-FLOOR(rdt))) SECOND
                ) AS data_timestamp, ".$input_param." AS data_value
                FROM ".$tablename." 
                WHERE DATE(    
                        DATE_ADD(
                            DATE_ADD(
                    ...
                    )= '".$data_date."' AND ".$input_param." IS NOT NULL AND ".$input_param." NOT IN (0, -998, -999) ORDER BY stack_id, data_timestamp";
            $this->log_debug(__FUNCTION__, __LINE__, $sql);
            $stmt = $db_old->query($sql);
            ...
            $sql = "SELECT 
                ...
                WHERE DATE(    
                    ...
                    )= '".$data_date."' GROUP BY stack_id";
            $this->log_debug(__FUNCTION__, __LINE__, $sql);
            $stmt = $db_old->query($sql);
            $stack_list = $stmt->fetchAll(PDO::FETCH_ASSOC); 
            return array('stack'=>$stack_list, 'data'=>$result);
        }
        catch(PDOException $e) {
            $this->log_debug(__FUNCTION__, __LINE__, 'Error Code - '.$e->getCode().' - '.$e->getMessage());
            if ($e->getCode() == '42S02')
                throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $e->getMessage()), 42);
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $e->getMessage()), $e->getCode());
        }
    }
    ...
    public function db_count_2($tablename, $columns=array())
    {
        try {
            if (!empty($this->DBH)){
                $where_str = "";
